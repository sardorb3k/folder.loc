...
class DashboardRepository implements DashboardRepositoryInterface
{
    ...
    ORDER BY day  DESC LIMIT 30");
        ...
        $amount = collect(\DB::select("SELECT SUM(amount) as amount, ( ROUND( ( SUM(amount) -( SELECT SUM(amount) FROM `payments` WHERE YEAR(`payment_date`) = 2022 AND MONTH(`payment_date`) = 11 ) ) /( SELECT SUM(amount) FROM `payments` WHERE YEAR(`payment_date`) = 2022 AND MONTH(`payment_date`) = 11 ) * 100 ) ) AS pres FROM `payments` WHERE YEAR(`payment_date`) = 2022 AND MONTH(`payment_date`) = 12"))->first();
        $user = collect(\DB::select("SELECT count(id) as count, ( ROUND( ( count(id) -( SELECT count(id) FROM `users` WHERE YEAR(`created_at`) = 2022 AND MONTH(`created_at`) = 11 ) ) /( SELECT count(id) FROM `users` WHERE YEAR(`created_at`) = 2022 AND MONTH(`created_at`) = 11 ) * 100 ) ) AS pres FROM `users` WHERE YEAR(`created_at`) = 2022 AND MONTH(`created_at`) = 12 and `status` = 'active'"))->first();
